<?php
	...
	if (isset($_POST['button']))
	{
		...
		for($i=0;$i < count($_POST['kriteria']); $i++){
			...
			$query = mysqli_query("SELECT * FROM nilai WHERE kode_alternatif = '$_POST[kode_alternatif]' AND kode_kriteria = '$kode_kriteria'");
			if (!$data = mysqli_fetch_array($query))
			{
				mysqli_query("INSERT INTO nilai(id_nilai, kode_alternatif, kode_kriteria, nilai) VALUES('0','$_POST[kode_alternatif]', '$kode_kriteria', '$nilai')");
			}
			mysqli_query("UPDATE nilai SET nilai = '$nilai' WHERE kode_alternatif = '$_POST[kode_alternatif]' AND kode_kriteria = '$kode_kriteria'");
